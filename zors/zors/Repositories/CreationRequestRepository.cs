...
namespace zors.Repositories
{
    public class CreationRequestRepository : ICreationRequestRepository
    {
        public List<Employee> SelectEmployeesByMbr(OracleConnection connection, string mbr)
        {
            ...
            connection.Open();
            comm = new OracleCommand("select ime, mbr, strucnasprema, obrazovanje, zanimanje, SMERODSEK, MESTOSTUDIRANJA, DATUMDIPLOMIRANJA, PROSECNAOCENA, OCENADIPLOMSKI, NAPOMENA, celina, koefzarade from ECR1 where MBR = '" + this.TextBox3.Text + "'");
            ...
            while (read.Read())
            {
                Employee employee = new Employee
                {
                      ...
                      EmployeeID = Convert.ToString(read[1]),
                    ...
                    //EmployeeProfessionalQualification = Convert.ToString(read[4]),
                    //EmployeeCurrentDepartmentID = Convert.ToString(read[5]),
                    //OrganisationDepartmentNameSer = Convert.ToString(read[6]),
                    //OrganisationDepartmentNameEng = Convert.ToString(read[7]),
                    //EmployeeEducationReference = Convert.ToString(read[8]),
                    //EmployeeCurrentDepartmentID = Convert.ToString(read[9]),
                    //JobID = Convert.ToString(read[10]),
                    //EmployeeCurrentCoefficient = Convert.ToString(read[11]),                    
                };
         ...
         public List<Organization> SelectOrganizationByMbr(OracleConnection connection, string mbr)
        {
            ...
            connection.ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["ConnectionStringUSSKAD"].ConnectionString;
            connection.Open();
            comm = new OracleCommand("select  **********  from ECR1 where MBR = '" + this.TextBox3.Text + "'");
            comm.Connection = connection;
            adapter = new OracleDataAdapter(comm);
            results = new DataSet();
            adapter.Fill(results);
